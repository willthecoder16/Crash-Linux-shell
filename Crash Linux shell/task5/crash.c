...
void eval(const char **toks, bool bg) {
    ...
    if (isJobId) {
        ...
        for (int i = 0; i < jobCount; i++) {
            if (jobs[i].jobId == fgJobId) {
                if (jobs[i].isSuspended && !jobs[i].isKilled){
                    kill(jobs[i].pid, SIGCONT);  // Resume the job
                    jobs[i].isSuspended = false;
                    printf("[%d] (%d)  continued  %s\n", jobs[i].jobId, jobs[i].pid, jobs[i].cmd);
                    jobs[i].isNotFinished = true;
        ...
        for (int arg = 1; toks[arg] != NULL; arg++) {  // Iterate over all arguments
            bool isJobId = toks[arg][0] == '%';
            bool jobFound = false;

            if (isJobId) {
                // Extract job ID, checking for valid integer
                char* endptr;
                long bgJobId = strtol(&toks[arg][1], &endptr, 10);
                if (*endptr != '\0') {
                ...
                // Search for the job by job ID
                for (int i = 0; i < jobCount; i++) {
                    if (jobs[i].jobId == bgJobId && !jobs[i].isKilled) {
                        jobFound = true;
                        // Resume the job
                        kill(jobs[i].pid, SIGCONT);
                        jobs[i].isSuspended = false;  // Mark the job as running
                        printf("[%d] (%d)  continued  %s\n", jobs[i].jobId, jobs[i].pid, jobs[i].cmd);
                        break;
                    }
                }
                ...
                // Extract process ID, checking for valid integer
                char* endptr;
                long bgPid = strtol(toks[arg], &endptr, 10);
                if (*endptr != '\0') {
                    fprintf(stderr, "ERROR: bad argument for bg: %s\n", toks[arg]);
                    continue;  // Skip to the next argument
                }
                ...
                // Search for the job by PID
                for (int i = 0; i < jobCount; i++) {
                    if (jobs[i].pid == bgPid && !jobs[i].isKilled) {
                        jobFound = true;
                        // Resume the job
                        kill(jobs[i].pid, SIGCONT);
                        jobs[i].isSuspended = false;  // Mark the job as running
        ...
        for (int i = 0; i < jobCount; i++) {
            if (!jobs[i].isKilled){
            printf("[%d] (%d)  %s  %s\n", jobs[i].jobId, jobs[i].pid, jobs[i].isSuspended ? "suspended" : "running", jobs[i].cmd);            
            }
        }
        return;  
    ...
    return;
        ...
        if (pid == 0) {  // Child process
            ...
            jobs[jobCount].isSuspended = false;
